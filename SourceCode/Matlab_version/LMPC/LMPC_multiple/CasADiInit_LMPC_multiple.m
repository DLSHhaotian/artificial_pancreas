function solver = CasADiInit_LMPC_multiple(N,T,Ad,Bd,Bd_d,Css,xs_MVP,us_MVP,z_min,z_max,z_ref,u_min,u_max,Wz,Weta_min,Weta_max,Wu_ba,Wu_delta_ba,Wu_bo,Wu_glu)
% Syntax: solver = CasADiInit_LMPC_single(N,T,Ad,Bd,Bd_d,Css,xs_MVP,us_MVP,z_min,z_max,z_ref,u_min,u_max,Wz,Weta_min,Weta_max,Wu_ba,Wu_delta_ba,Wu_bo)
...
u_1 = [0;0;0];
u0 = zeros(N,num_u); 
OPT_var_x0 = [reshape(u0',num_u*N,1);zeros(2*N,1)];
...
gglu = zeros(N*num_u+2*N*num_eta,1);
for i=1:N
    %fill the Phi
    ...
    gbo((i-1)*num_u+2,1) = Wu_bo;
    H_bo(i*num_u,i*num_u) = Wu_bo;
    gglu((i)*num_u,1) = Wu_glu;
end
Mx0=Gam'*Qz*Phi;
Md=Gam'*Qz*Phi_d;
...
g=Mx0*x_bar+Mr*(z_ref*ones(N,1))+Mu_delta*u_1+Md*dmeal+gbo+gglu;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Az_cons_min=Gam;
...
qp = struct ;
qp.h = Hu_DM.sparsity();
qp.a = A_ieq_DM.sparsity();
solver = conic('solver','qpoases',qp);
end