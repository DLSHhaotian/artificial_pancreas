function [solver,args] = CasADiInit_NMPC_multiple(N,T,p_MVP,us_MVP,z_max,z_min,z_ref,u_min,u_max,Wz,Wz_max,Wz_min,Wu_bo,Wu_ba,Wu_glu)
% Syntax: [solver,args] = CasADiInit_NMPC_multiple(N,T,p_MVP,us_MVP,z_max,z_min,z_ref,u_min,u_max,Wz,Wz_max,Wz_min,Wu_bo,Wu_ba,Wu_glu)
...
R_glu=Wu_glu;% weighing matrices (controls glucagon)
uba_ref = us_MVP(1);
...
for k=1:N
    st=X(:,k); zk=st(4);
    con=U(:,k); 
    obj = obj+(zk-P(n_states+1))*Qz*(zk-P(n_states+1)) + myMax(zk-z_max,0)*Qz_max*myMax(zk-z_max,0) + myMin(zk-z_min,0)*Qz_min*myMin(zk-z_min,0)+R_bo*myAbs(con(2))+R_glu*myAbs(con(3));
    if k~=N
        %delta uba 2-norm
%          con_next=U(:,k+1);
%          obj=obj+(con_next(1)-con(1))*R_delta_ba*(con_next(1)-con(1));
    end
    %uba-uba_bar 2-norm
