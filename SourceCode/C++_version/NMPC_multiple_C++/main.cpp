...
typedef Eigen::Matrix<double,1,9> Matrix1_9d;
typedef Eigen::Matrix<double,9,1> Matrix9_1d;

int main() {
    const int t_final = 24*60, T = 5, N = 72;
    const int N_all = t_final/T+1;
    const int n_statesHovk = 16,n_statesMVP =9,n_controls = 3,n_disturbance = 1;
    const int HR_base = 70;
    double z_ref = 108;
    double x0_Hovk[n_statesHovk] = {0.0,0.0,664.58396,664.58396,63.61117887,36.02065,6.28366995,0.0376654,0.0052789,0.14319,6.0,0.0,0.0,0.0,100.0,0.0};
    double x0_MVP[n_statesMVP] = {10.043388,10.043388, 0.01999, 108.0000, 0, 0, 108.0000,0.0,0.0};
    double t_arr[N_all], x_arr[n_statesHovk*N_all],y_arr[N_all],z_arr[N_all],u_arr[(N_all-1)*n_controls],d_arr[(N_all-1)*n_disturbance],HR_arr[(N_all-1)*n_disturbance],t_cost[N_all-1];
    ...
    double yk = 0, zk = 0, zk_bar = 0, xk[n_statesHovk],xk_bar[n_statesMVP] ,uk[3] = {0,0,0},uk_d[4] = {0,0,0,0};
    ...
    for(;k<N_all-1;++k){
        ...
        uk_d[0] = uk[0], uk_d[1] = uk[1], uk_d[2] = uk[2],uk_d[3] = d_arr[k]*1000;
        for(int i=0;i<n_statesHovk;++i){
            //dW_k[i] = dW[i][k];
            dW_k[i] = 0;
        }
        ...
        Matrix9_1d xk_bar_plus = K_kf*e_kf;
        for(int i=0;i<n_statesMVP;++i){
        ...
        uk[0] = usol[0], uk[1] = usol[1]; uk[2] = usol[2];
        ...
        if(d_arr[k]>0){
            ...
            cout<<"The uk:"<<uk[0]<<", "<<uk[1]<<", "<<uk[2]<<endl;
        }
        sol.clear();
        ...
        //xk_vec = explicitRK4::explicitRK4_Hovorka(T,xk,uk,d_arr[k],HR_arr[k],dW_k,p_sigma);
        xk_vec = explicitRK4::Dopri5_Hovorka(T,xk,uk,d_arr[k],HR_arr[k],dW_k,p_sigma);
