...
% Linear model predictive control(LMPC) for single-hormone 
...
% --------------------------------------------------------------
syms x1 x2 x3 x4 x5 x6 x7 u1 u2 d1 
...
fx(4,1) = -x3*x4+p_MVP(4)+(x6*p_MVP(5)/p_MVP(6));
...
Bss=eval(vpa(subs(jacobian(fx,F(1:2)),[X;F],[xs_MVP;us_MVP;ds]),4));
Bdss=eval(vpa(subs(jacobian(fx,F(3)),[X;F],[xs_MVP;us_MVP;ds]),4));
...
solver = CasADiInit_LMPC_single(N,T,Ad,Bd,Bd_d,Css,xs_MVP,us_MVP,z_min,z_max,z_ref,u_min,u_max,Wz,Weta_min,Weta_max,Wu_ba,Wu_delta_ba,Wu_bo);
...
% --------------------------------------------------------------
[x_arr,z_arr,y_arr,u_arr,d_arr,Z_bar,t_cost] = ClosedLoopSimulation_LMPC_single(Ad,Bd,Bd_d,Css,solver,xs_Hovorka,xs_MVP,us_MVP,ys,N,N_all,T,t,p_Hovorka,p_MVP,addSensorNoise,dW,addProcessNoise,z_min,z_max,z_ref,u_min,u_max,Wz,Weta_min,Weta_max,Wu_ba,Wu_delta_ba,Wu_bo);
